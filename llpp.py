...
def ask_rup_digitale(user_message: str, system_prompt: str, history: list) -> str:
    ...
    if not api_key:
        return "[Errore configurazione: manca GEMINI_API_KEY nei Secrets di Streamlit.]"
    ...
    try:
        ...
        return response.text if hasattr(response, "text") else "[Risposta vuota dal modello]"
    except Exception as e:
...
with st.sidebar:
    ...
    st.caption("Motore: Gemini 2.5 Flash | Riferimento: D.Lgs. 36/2023")
    st.markdown("---")
...
if "system_prompt" not in st.session_state:
    st.session_state.system_prompt = SYSTEM_PROMPT_TEMPLATE.replace("{DOCUMENTI_LETTI}", "[Nessun documento]")
...
for msg in st.session_state.messages:
    ...
    if msg["role"] == "assistant" and msg.get("has_draft", False):
        st.download_button(
            label="📥 Scarica Bozza (.md)",
            data=msg["content"],
            file_name=f"bozza_{msg['id']}.md",
            mime="text/markdown",
            key=f"dl_{msg['id']}"
        )
    ...
    if not st.session_state.context.strip():
        ...
        if has_draft:
            st.download_button(
                label="📥 Scarica Bozza (.md)",
                data=reply,
                file_name=f"bozza_{datetime.now().strftime('%Y%m%d_%H%M%S')}.md",
                mime="text/markdown",
